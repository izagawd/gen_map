...
pub unsafe trait KeyLayout<Idx: KeyPiece, Gen: KeyPiece> {
    ...
    /// The largest generation a key can hold. It is odd.
    fn max_generation() -> Gen;

    /// Packs an index and a generation.
    ///
    /// # Safety
    ///
    /// `idx` must be at most [`max_idx`](Self::max_idx), and `generation`
    /// must be odd and at most [`max_generation`](Self::max_generation).
    unsafe fn pack_unchecked(idx: Idx, generation: Gen) -> Self::Repr;

    /// The index that was packed.
    fn idx(repr: Self::Repr) -> Idx;

    /// The generation that was packed.
    fn generation(repr: Self::Repr) -> Gen::NonZero;
}
...
pub struct SplitRepr<Idx: KeyPiece, Gen: KeyPiece> {
    idx: Idx,
    /// The generation is odd, so it is never zero, and storing it this way
    /// gives `Option<Key>` the size of `Key`.
...
unsafe impl<Idx: KeyPiece, Gen: KeyPiece> KeyLayout<Idx, Gen> for Split {
    ...
    /// The largest value of an unsigned integer is odd.
    #[inline]
    fn max_generation() -> Gen {
        Gen::MAX
    }
    ...
    #[inline]
    unsafe fn pack_unchecked(idx: Idx, generation: Gen) -> SplitRepr<Idx, Gen> {
        debug_assert!(generation.is_odd());
        // SAFETY: the caller promises an odd generation, and zero is even.
        let generation = unsafe { generation.into_non_zero_unchecked() };
    ...
    #[inline]
    fn generation(repr: SplitRepr<Idx, Gen>) -> Gen::NonZero {
        repr.generation
    }
}
...
// read back from the same fields, and the largest generation has all of its
// bits set, so it is odd.
    ...
    for Packed<R, GEN_BITS>
{
    ...
    fn max_generation() -> Gen {
        check_packed::<Idx, Gen, R, GEN_BITS>();
        // SAFETY: the check makes sure `Gen` has at least `GEN_BITS` bits.
        unsafe { Gen::from_u128_unchecked(low_bits(GEN_BITS)) }
    }
